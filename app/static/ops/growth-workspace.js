(function () {
  ...
  function renderLaunchDeliveryControls() {
    ...
    const headerNote=rejectedExperiments.length?`其余 ${Math.max(0,experiments.length-rejectedExperiments.length)} 条保持当前状态；AI 已为拒审广告准备替代素材。`:(campaignPaused?`Meta 实时状态${liveChecked?` · 更新于 ${liveChecked}`:''}`:(creationIncident?'可沿用已批准方案，只补未完成步骤；所有新对象仍保持暂停。':(activationChecking?`${confirmedSteps} 项已确认 · ${uncertainSteps} 项待核对 · ${untouchedSteps} 项尚未执行`:(activationIncident?'系统已停止推进，等待明确的失败原因。':(runningCount?`审核中的广告将在审核通过后自动投放${liveChecked?` · 更新于 ${liveChecked}`:''}`:'开启后若仍在审核，审核通过将自动投放')))));
    const primaryAction=creationIncident&&launchState.batchPlan?.plan_id?`<button type="button" class="growth-launch-primary" data-launch-order-auto-recover${metaDisabled}>确认自动修复</button>`:(activationIncident?'<button type="button" class="growth-launch-primary" data-launch-activation-exception>查看原因和下一步</button>':((campaignPaused||canEnableOrder)?`<button type="button" class="growth-launch-primary" data-launch-enable-order${metaDisabled}>开启整单投放</button>`:''));
  ...
  async function openLaunchIncidentResolution(planId,payload,plan) {
    ...
    const primary=repairReady?'<button type="button" class="growth-primary" id="growthConfirmRepairPlan">使用所选主页继续创建</button>':(canResume?'<button type="button" class="growth-primary" id="growthResumeIncidentPlan">确认自动修复</button>':'<button type="button" class="growth-primary" id="growthRefreshIncident">重新检测</button>');
  ...
  async function confirmRecoverableCreationIncident(planId,button) {
    ...
    try{
      ...
      if(!payload.can_resume_same_plan){button.disabled=false;button.textContent=originalText;await openLaunchIncidentResolution(planId,payload,plan);return;}
      button.disabled=false;button.textContent=originalText;
      await resumeLaunchBatchSamePlan(planId,button);
    }catch(error){button.disabled=false;button.textContent=originalText;showModalError(readableError(error));}
  }
  ...
  function renderExperimentDetail(payload) {
    ...
    node.innerHTML = creationIncident?`
      <section class="growth-status-panel"><small>整单创建异常</small><h3>系统已停止旧任务，当前不会产生花费</h3><p>系统可以沿用你已批准的原方案，只补未完成步骤，不会重复已成功的写入。</p><div class="growth-status-next">你只需确认一次；确认后系统自动核对、续建并回读，所有新对象保持暂停。</div><div class="growth-status-actions">${planId?'<button type="button" class="growth-primary" id="growthConfirmOrderRecovery">确认自动修复</button>':'<button type="button" id="growthReloadOrderIncident">重新读取订单</button>'}</div></section>
  ...
  function closeBulkRebuildModal() {
    ...
    if(node){const returnFocus=node._bulkReturnFocus;node.hidden=true;node.innerHTML='';if(returnFocus&&returnFocus.isConnected)requestAnimationFrame(()=>returnFocus.focus());}
  }
  ...
  function bulkRebuildItemHtml(item,{compact=false}={}) {
    const status=String(item.status||'PENDING'),guidance=status==='MANUAL_REVIEW'?bulkRebuildErrorGuidance(item.error):null;
    return `<article class="growth-bulk-item is-${status.toLowerCase()}"><div class="growth-bulk-item-main"><strong>${esc(item.ad_name||item.ad_id)}</strong><small>${esc(item.account_name||item.account_id||'未命名账户')} · ${esc(item.ad_id||'无广告 ID')}</small></div><span class="growth-bulk-status is-${status.toLowerCase()}">${esc(bulkRebuildItemStatus(item))}</span>${compact?'':guidance?`<div class="growth-bulk-guidance"><b>${esc(guidance.title)}</b><span>${esc(guidance.detail)}</span>${guidance.raw?`<details><summary>技术详情</summary><code>${esc(guidance.raw)}</code></details>`:''}</div>`:''}${compact?bulkRebuildTaskButton(item,status==='SUCCESS'?'查看重建结果':'查看并处理'):bulkRebuildTaskButton(item,status==='MANUAL_REVIEW'?'查看并处理':'查看重建结果')}</article>`;
  }

  function bindBulkRebuildTaskButtons(node) {
  ...
  function renderBulkRebuildProgress(batch) {
    ...
    const started=success+manual+running>0,total=batch.items.length,processed=success+manual,progress=total?Math.round((processed/total)*100):0;
    const title=document.getElementById('growthBulkRebuildTitle'),subtitle=document.getElementById('growthBulkRebuildSubtitle'),primary=document.getElementById('growthConfirmBulkRebuild'),footnote=document.getElementById('growthBulkRebuildFootnote');
    if(title)title.textContent=started?'批量重建进度':'确认批量重建';
    if(subtitle)subtitle.textContent=started?`${processed}/${total} 条已处理 · 批次 ${batch.batch_id.slice(-8)}`:`${total} 条广告 · 全部以暂停态创建`;
    if(footnote)footnote.textContent=started?'关闭窗口不会取消已提交的请求':'开始前不会向 Meta 写入任何内容';
    if(primary){primary.hidden=!pending;if(!primary.disabled)primary.textContent=started?`继续处理 ${pending} 条`:`开始重建 ${pending} 条广告`;}
    if(!started){
      const accountCount=new Set(batch.items.map(item=>String(item.account_id||item.account_name||'')).filter(Boolean)).size;
      node.innerHTML=`<section class="growth-bulk-confirm"><div class="growth-bulk-scope"><div><small>本次范围</small><strong>${total} 条广告</strong><span>${accountCount} 个广告账户</span></div><div><small>创建结果</small><strong>全部暂停</strong><span>Meta 写入后不会启用</span></div><div><small>异常处理</small><strong>立即停止</strong><span>不自动重试</span></div></div><div class="growth-bulk-orientation"><b>确认后系统会做什么</b><ol><li>逐条读取并冻结原广告配置</li><li>审批方案并完成安全演练</li><li>创建暂停态对象并回读核对</li></ol></div><details class="growth-bulk-group"><summary><span>查看将处理的广告</span><strong>${total}</strong></summary><div class="growth-bulk-group-list">${batch.items.map(item=>bulkRebuildItemHtml(item,{compact:true})).join('')}</div></details></section>`;
      bindBulkRebuildTaskButtons(node);return;
    }
    const manualItems=batch.items.filter(item=>item.status==='MANUAL_REVIEW'),runningItems=batch.items.filter(item=>item.status==='RUNNING'),successItems=batch.items.filter(item=>item.status==='SUCCESS'),pendingItems=batch.items.filter(item=>item.status==='PENDING');
    node.innerHTML=`<section class="growth-bulk-progress"><div class="growth-bulk-summary" aria-label="批次状态"><div class="is-success"><small>已完成</small><strong>${success}</strong></div><div class="is-manual"><small>需复核</small><strong>${manual}</strong></div><div class="is-running"><small>处理中</small><strong>${running}</strong></div><div><small>排队中</small><strong>${pending}</strong></div></div><div class="growth-bulk-progress-line"><span><b>总体进度 ${progress}%</b><small>${processed} / ${total} 条已有明确结果</small></span><div role="progressbar" aria-label="批量重建进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${progress}"><i style="width:${progress}%"></i></div></div>${manualItems.length?`<section class="growth-bulk-priority"><header><div><small>优先处理</small><h3>${manualItems.length} 条结果需要你复核</h3></div><span>系统已停止这些条目</span></header><div class="growth-bulk-priority-list">${manualItems.map(item=>bulkRebuildItemHtml(item)).join('')}</div></section>`:''}${runningItems.length?`<section class="growth-bulk-running"><header><b>正在处理</b><span>请保持页面打开</span></header>${runningItems.map(item=>bulkRebuildItemHtml(item,{compact:true})).join('')}</section>`:''}${successItems.length?`<details class="growth-bulk-group"><summary><span>已完成的广告</span><strong>${successItems.length}</strong></summary><div class="growth-bulk-group-list">${successItems.map(item=>bulkRebuildItemHtml(item,{compact:true})).join('')}</div></details>`:''}${pendingItems.length?`<details class="growth-bulk-group"><summary><span>等待处理的广告</span><strong>${pendingItems.length}</strong></summary><div class="growth-bulk-group-list">${pendingItems.map(item=>bulkRebuildItemHtml(item,{compact:true})).join('')}</div></details>`:''}</section>`;
    bindBulkRebuildTaskButtons(node);
  }
  ...
  function showBulkRebuildModal(batch,{allowPending=true,persisted=true}={}) {
    ...
    node._bulkReturnFocus=document.activeElement;
    node.innerHTML=`<section class="growth-modal" role="dialog" aria-modal="true" aria-labelledby="growthBulkRebuildTitle"><header class="growth-modal-head"><div><b id="growthBulkRebuildTitle">确认批量重建</b><small id="growthBulkRebuildSubtitle">${batch.items.length} 条广告 · 全部以暂停态创建</small></div><button type="button" class="growth-icon-button" data-bulk-modal-close aria-label="关闭批量重建窗口">×</button></header><div class="growth-modal-body"><div class="growth-safety growth-bulk-safety"><b>安全边界</b><span>只重建投放配置；不会暂停现有广告、调预算、放量、替换素材或开启新广告。每条广告独立回读，结果不确定时立即停止且不自动重试。</span></div>${persisted?'':'<div class="growth-error">浏览器无法保存批次进度，本次尚未执行任何广告操作。请清理站点存储空间或更换正常浏览器窗口后重试。</div>'}<div id="growthBulkRebuildProgress" role="status" aria-live="polite"></div></div><footer class="growth-modal-foot"><span class="growth-bulk-footnote" id="growthBulkRebuildFootnote">开始前不会向 Meta 写入任何内容</span><button type="button" data-bulk-modal-close>${pending===batch.items.length?'取消':'关闭'}</button>${persisted&&allowPending&&pending?`<button type="button" class="growth-primary" id="growthConfirmBulkRebuild">${pending===batch.items.length?'开始重建':'继续处理'} ${pending} 条广告</button>`:''}</footer></section>`;
    node.hidden=false;
    ...
    requestAnimationFrame(()=>{const target=node.querySelector('#growthConfirmBulkRebuild:not([hidden])')||node.querySelector('[data-bulk-modal-close]');if(target)target.focus();});
  }
