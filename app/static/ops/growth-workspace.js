(function () {
  ...
  function installStyles() {
    ...
    integratedShell.textContent = `
      ...
      .growth-launch-delivery-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap}.growth-launch-delivery-actions button{white-space:nowrap}
  ...
  const launchState={screen:'orders',experimentMode:'creative_direction',audiencePreview:null,approvedCreatives:[],frozenCreativeId:'',copyVariants:[],audienceRound:0,launchArchiveView:false,launchOrderFilter:'all',taskHomeView:'orders',taskSearch:'',taskIndexLoading:false,target:{country:'BR',app:'Tugao',daily:'200',cpi:'0.30',gender:'female',age_min:'18',age_max:'40',language:'pt_BR',account_id:'',account_name:'',page_id:'',page_name:''},accounts:[],pages:[],countryPageIds:{},accountPageIds:{},accountPageOptions:{},directions:[],historyEvidence:{},namingRule:{},namingDate:'',regenerationRound:0,directionsLoading:false,accountsLoading:false,accountsLoaded:false,launches:[],launchesLoaded:false,launchesLoading:false,ordersNotice:'',orderRetryTimer:null,metaRateLimit:{loading:false,blocked:false,unknown:false,account_id:'',retry_after_seconds:0,blocked_until:0,reason:''},metaRateLimitTimer:null,metaRateLimitCountdownTimer:null,deliveryStatus:{loading:false,data:null,error:''},deliveryStatusTimer:null,launch:null,batchPlan:null,jobs:[],experiments:[],experimentStates:[],orderPhase:'',orderStatusZh:'',orderDataMismatch:false,approved:false,lastNotifiedPhase:'',lastProgressSignature:'',error:''};
  ...
  function confirmPermanentDeleteLaunch(launchId) {
    ...
    document.getElementById('growthPermanentDeleteConfirm')?.addEventListener('click',async event=>{
      ...
      try{
        ...
        closeModal();await Promise.all([loadLaunchOrders(),loadUnifiedTaskIndex()]);showLaunchToast(deletingMeta?'订单及其独占 Meta 对象已删除。':'订单已永久删除，Meta 对象保持不变。');
  ...
  function renderLaunchOrders() {
    ...
    const rows=visibleOrders.map(order=>{
      ...
      return `<article class="growth-launch-order-card ${order.archived?'is-archived':statusClass(order.phase)}"><div class="growth-launch-order-main"><div class="growth-launch-order-title"><b>${esc(launchCampaignName(order))}</b><span class="growth-launch-order-status ${statusClass(order.phase)}">${esc(order.archived?'已归档':(order.status_zh||'处理中'))}</span></div><div class="growth-launch-order-context">${esc(launchAccountName(order))} · ${esc(String(target.country||'-').toUpperCase())} · ${esc(audienceText||'受众待确认')}</div><div class="growth-launch-order-facts"><div><small>经营目标</small><b>$${esc(String(target.daily_spend_target||'-'))}/天 · CPI ≤ $${esc(String(target.cpi_target||'-'))}</b></div><div><small>素材</small><b>${completed}/${esc(String(order.experiment_count||0))} 已通过${pendingReview?` · ${pendingReview} 待审`:''}</b></div><div class="is-next"><small>下一步</small><b>${esc(nextAction(order))}</b></div></div><div class="growth-launch-order-foot"><span>${order.archived?'归档于':'更新于'} ${esc(formatTime(order.archived_at||order.updated_at))}${retentionText}</span></div></div><div class="growth-launch-order-actions">${actions}</div></article>`;
    }).join('');
    const emptyCopy={
  ...
  function renderLaunch(screen,options={}) {
    ...
    if(screen==='cold') node.innerHTML=launchLayout(['audience_strategy','copy_variant'].includes(launchState.experimentMode)?renderAudienceLaunchReady():renderColdLaunch(),'new');
  ...
  function renderColdLaunch() {
    ...
    const order=launchState.launch||{},campaignName=launchCampaignName(order),modeLabel=launchModeLabel(order);
    if(launchState.orderDataMismatch)return `<header class="growth-launch-head growth-launch-cold-head"><div class="growth-launch-order-identity"><span>广告订单</span><h1>${esc(campaignName)}</h1><p>${esc(String(launchState.target.country||'-').toUpperCase())} · Tugao · ${esc(modeLabel)} · $${esc(launchState.target.daily)}/天 · CPI ≤ $${esc(launchState.target.cpi)}</p></div><button type="button" class="growth-launch-secondary" data-launch-view-target>查看目标</button></header><section class="growth-launch-cold"><div class="growth-launch-cold-status"><div><span class="growth-launch-kicker">系统核对中</span><h2>订单与广告信息暂未匹配</h2><p>系统已隐藏投放状态和操作，正在按当前订单重新读取；不会操作其他订单的广告。</p></div></div><div class="growth-launch-delivery-note">请刷新广告任务后重试；订单归属核对通过前不能开启或关闭广告。</div></section>`;
    ...
    return `<header class="growth-launch-head growth-launch-cold-head"><div class="growth-launch-order-identity"><span>广告订单</span><h1>${esc(campaignName)}</h1><p>${esc(String(launchState.target.country||'-').toUpperCase())} · Tugao · ${esc(modeLabel)} · $${esc(launchState.target.daily)}/天 · CPI ≤ $${esc(launchState.target.cpi)}</p></div><button type="button" class="growth-launch-secondary" data-launch-view-target>查看目标</button></header><section class="growth-launch-cold">${workflowState?renderLaunchDeliveryControls():`${materialHeader}${progressHtml}`}${!workflowState&&primaryLabel?`<footer class="growth-launch-cold-actions"><button type="button" class="growth-launch-primary" data-launch-material>${primaryLabel}</button></footer>`:''}</section>`;
  }
  ...
  async function approveLaunchMaterial(imageId,button) { try{button.disabled=true;button.textContent='正在确认…';await api(`/api/ops/ad-data-dashboard/creative-images/${encodeURIComponent(imageId)}/review`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({review_status:'APPROVED',checks:{feed_static_ad_structure:true,simulation_review:true}})});const launchId=String(launchState.launch?.launch_id||'');closeModal();await refreshLaunchDeliveryOrder(launchId);showLaunchToast('素材已通过审核');}catch(error){button.disabled=false;button.textContent='通过';const message=`审核失败：${readableError(error)}`;showModalError(message);showLaunchToast(message);} }
  async function regenerateLaunchMaterial(jobId,button) { if(!window.confirm('确认生成一个新版本？当前已通过素材会保留，不会删除或失效。'))return;try{button.disabled=true;button.textContent='正在创建新版本…';await api(`/api/ops/creative-pro-jobs/${encodeURIComponent(jobId)}/start-generation`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({image_size:'1024x1024',candidate_count:1,max_attempts:3,force_regenerate:true})});const launchId=String(launchState.launch?.launch_id||'');closeModal();await refreshLaunchDeliveryOrder(launchId);showLaunchToast('新版本已开始生成，当前素材继续保留');}catch(error){button.disabled=false;button.textContent='重做';const message=`重新生成失败：${readableError(error)}`;showModalError(message);showLaunchToast(message);} }
  ...
  function renderLaunchDeliveryControls() {
    ...
    const rows=experiments.map((experiment,index)=>{
      ...
      const adPerformance=experiment.performance||{},rowMetrics=adPerformance.available?`消耗 ${metricMoney(adPerformance.spend)} · 安装 ${metricNumber(adPerformance.installs)} · CPI ${metricMoney(adPerformance.cpi)} · CTR ${metricPercent(adPerformance.ctr)} · 入会 ${metricNumber(adPerformance.real_bind_count)}`:'暂未回流广告级数据';
      return `<div class="growth-launch-delivery-row"><div class="growth-launch-delivery-copy"><b>${esc(label)}</b><span>${esc(experiment.experiment_code||`广告 ${index+1}`)}</span><small class="growth-launch-row-metrics">${esc(rowMetrics)}</small></div><span class="growth-launch-delivery-status ${statusClass}">${status}</span>${action?`<button type="button" class="${running?'growth-launch-secondary':'growth-launch-primary'}" data-launch-delivery-experiment="${esc(experiment.experiment_id)}" data-launch-delivery-action="${running?'pause':'enable'}"${metaDisabled}>${action}</button>`:''}</div>`;
    }).join('');
    ...
    const refreshAction=`<button type="button" class="growth-launch-secondary" data-launch-refresh-delivery${launchState.deliveryStatus?.loading?' disabled':''}>${launchState.deliveryStatus?.loading?'同步中…':'刷新状态'}</button>`;
    const headerAction=`<div class="growth-launch-delivery-actions">${refreshAction}${primaryAction}</div>`;
  ...
  async function openLaunchBatchWorkflow(planId,knownPlan={}) {
    try{
      ...
      const audienceMode=String(plan.test_variable||'')==='audience_strategy',planAllowsLive=plan.execution_policy?.live_creation_allowed!==false,liveAvailable=payload.live_execution_available===true,liveAllowed=planAllowsLive&&liveAvailable;
      const dryVerified=String(dry.status||'')==='DRY_RUN_VERIFIED',taskStatus=String(task.status||''),created=taskStatus==='SUCCESS',canResume=Boolean(payload.can_resume_same_plan)&&next==='RESUME_SAME_PLAN',externalPrerequisite=Boolean(payload.external_prerequisite_required);
      const objectIds=task.meta_object_ids_json||{};
      const rows=cells.map((cell,index)=>{const key=String(cell.cell_key||`C${index+1}`).toLowerCase(),adId=String(objectIds[`${key}_ad_id`]||'');return `<div><span>${esc(cell.role==='BASELINE'?'基':'挑')}</span><b>${esc(audienceMode?(cell.audience_strategy?.label||cell.audience_strategy?.strategy_key):(cell.creative_direction?.title||cell.experiment_code||`实验 ${index+1}`))}</b><span>${esc(cell.steps?.ADSET_CREATE?.name||'-')} · ${created?`已创建并暂停${adId?` · Ad ${esc(adId)}`:''}`:`$${esc(String((cell.steps?.ADSET_CREATE?.daily_budget||0)/100))}/天`}</span></div>`;}).join('');
      const writeStepCount=audienceMode?4+2*cells.length:1+4*cells.length;
      ...
      else if(taskStatus)statusText=`<b>${esc(taskStatus==='MANUAL_REVIEW'?'创建尚未完成':statusLabel(taskStatus))}</b><span>${taskStatus==='MANUAL_REVIEW'?'系统已保存成功步骤并停止重复写入。':'系统正在自动创建并回读，不需要重复提交。'}</span>`;
      ...
      const technical=taskStatus?`<details class="growth-batch-technical"><summary>查看技术记录</summary><span>任务状态：${esc(taskStatus)} · 当前步骤：${esc(task.current_step||'-')} · 回执：${receipts.length} 条</span></details>`:'';
      showModal(`<section class="growth-modal growth-batch-modal"><header class="growth-modal-head"><div><b>广告创建状态</b><small>${esc(plan.campaign?.name||'-')} · ${audienceMode?'受众策略':'素材方向'}</small></div><button type="button" class="growth-icon-button" data-modal-close aria-label="关闭">×</button></header><div class="growth-modal-body"><div class="growth-mini-steps growth-mini-steps-compact"><span class="active">方案确认</span><span class="${dryVerified?'active':''}">安全检查</span><span class="${taskStatus||created?'active':''}">创建结果</span></div><div class="growth-batch-status">${statusText}</div><div class="growth-batch-ledger">${rows}</div>${technical}</div><footer class="growth-modal-foot"><button type="button" data-modal-close>关闭</button>${created?'<button type="button" class="growth-primary" id="growthViewCreatedOrder">查看订单状态</button>':''}${pageMismatch?'<button type="button" class="growth-primary" id="growthRepairBatchPage">修改公共主页</button>':''}${!pageMismatch&&!dryVerified&&next!=='PLAN_EXPIRED_REPLAN'?'<button type="button" class="growth-primary" id="growthRunBatchDry">运行安全检查</button>':''}${next==='SUBMIT_PAUSED_OBJECT_CREATION'&&liveAllowed?'<button type="button" class="growth-primary" id="growthCreateBatchPaused">创建广告</button>':''}${canResume?`<button type="button" class="growth-primary" id="growthResumeSamePlan">${externalPrerequisite?'自动核对并继续':'继续当前订单'}</button>`:''}${taskStatus&&!created&&!canResume&&!pageMismatch?'<button type="button" id="growthRefreshBatchStatus">查看最新结果</button>':''}</footer></section>`);
      ...
      document.getElementById('growthRefreshBatchStatus')?.addEventListener('click',()=>openLaunchBatchWorkflow(planId));
  ...
  function openPlanReadiness(experiment,payload) {
    ...
    const costCapUsd=Number(hypothesis.cpi_target||variant.cpi_target||0);
    ...
    if(actionType==='CREATE_PAUSED_AD'&&!(costCapUsd>0))blockers.push('CPI 成本上限');
    ...
    if(button&&!blockers.length)button.addEventListener('click',async()=>{
      ...
      if(actionType==='CREATE_PAUSED_AD'){
        ...
        if(!campaignName||!adsetName||!adName||!primaryText||!headline||!(dailyBudget>=5&&dailyBudget<=100)){showModalError('请补齐名称、文案，并把广告组每日预算设置在 5–100 美元之间。');return;}
        before.objects=[];
        after.status='PAUSED';
  ...
  function recommendationExperimentType(recommendation, selectedAction) {
    ...
    if(raw==='generate_creative'||String(recommendation.diagnosis_type||'').includes('creative'))return 'CREATIVE_REPLACEMENT';
    return 'NEW_AD_TEST';
  }
  ...
  function recommendationDraft(recommendation, selectedAction) {
    ...
    return {
      ...
      hypothesis_json:{summary:String(recommendation.reason_zh||recommendation.diagnosis_type_zh||'验证系统优化建议'),recommended_action:selectedAction,evidence_only:true},
  ...
  function closeModal() { ['growthModal','growthLaunchModal'].forEach(id=>{const node=document.getElementById(id);if(node){node.hidden=true;node.innerHTML='';}}); }
